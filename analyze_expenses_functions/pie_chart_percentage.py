...
def pie_chart_percentage(username):

    print("Viewing percentage of spending for each category within a period...\
    ...
    if category_count.sum() < 1: #if not enough data points
        ...
        labels = (df.index).tolist() #puts filtered category names into list
        ...
        plt.pie(df, labels=labels, radius=4, startangle=90, textprops=textprops, autopct='%1.1f%%',\
